...
<div class="panel"><h2>Experiment status and data freshness</h2><table><thead><tr><th>Model</th><th>Method</th><th>Assigned</th><th>Successful progress</th><th>Results recorded</th><th>Last reporter check</th><th>Data state</th><th>Success</th><th>Running</th><th>Failed</th><th>Not started</th><th>Refresh</th></tr></thead><tbody>${c.cells.map(x=>`<tr><td>${esc(x.model)}</td><td>${esc(x.method)}</td><td>${esc(x.assigned_owner)} · ${esc(x.assigned_machine)}</td><td>${progressBar(x)}</td><td>${x.recorded}/${x.expected}</td><td>${esc(when(x.last_reported_at))}</td><td class="${esc(x.report_state)}">${esc(x.report_state)}</td><td class="ok">${x.counts.succeeded}</td><td class="running">${x.counts.running}</td><td class="failed">${x.counts.failed}</td><td class="pending">${x.counts.pending}</td><td>${refreshButton(x)}</td></tr>`).join('')}</tbody></table></div>
