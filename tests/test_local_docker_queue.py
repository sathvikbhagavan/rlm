...
def test_local_docker_queue_has_exact_order_and_cardinalities() -> None:
    assert [phase.name for phase in PHASES] == [
        ...
        "gemini-repair",
        "deepseek-docker",
        "deepseek-rlm-x1000-docker",
        "glm-docker",
    ]
    assert [phase.expected_runs for phase in PHASES] == [6, 30, 8, 24, 15, 45]
...
def test_local_queue_never_requests_failed_retries() -> None:
    source = CODE_ROOT / "experiments/iclr2027/run_local_docker_queue.py"
