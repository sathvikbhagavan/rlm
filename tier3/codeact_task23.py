...
def maybe_init_tracing() -> None:
    ...
    initialized = init_tracing(
        project_name="CodeAct-Task23_tier3",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CodeAct task 23 evaluation.")
...
async def main(model_name: str, context_size: int) -> None:
    maybe_init_tracing()
    tracer = get_tracer("codeact-task23")
    ...
    run_session_id = f"codeact-task23-{uuid.uuid4()}"
    question = build_question()
    full_gt_set = set(TASK23_HARDCODED_GROUND_TRUTH_INDICES)
    ...
    with tracer.start_as_current_span("codeact_task23_sample_0") as sample_span:
        ...
        with using_tracing_attributes(
            ...
            tags=["codeact", "sample", "task23_stereocenter_from_achiral_reactants"],
        ):
    ...
    run.summary["ground_truth/stereocenter_from_achiral_reactants/full_count"] = len(full_gt_set)
