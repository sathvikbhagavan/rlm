...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run LLM task 12 C-O bond breaking index evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
def build_question() -> str:
    ...
    - Count how many times each C-O bond type appears on each side.
    - A reaction matches when reactants contain more instances of at least one C-O bond type than products.
...
async def main(model_name: str, context_size: int) -> None:
    ...
    with using_tracing_attributes(
        ...
        tags=["llm-baseline", "sample", "task12_co_bond_breaking"],
    ):
