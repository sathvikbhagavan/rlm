...
from task19_hardcoded_ground_truth import (
    TASK19_GROUND_TRUTH_DEFINITION,
    TASK19_HARDCODED_GROUND_TRUTH_INDICES,
    TASK19_POSITIVE_REACTIONS,
    TASK19_QUINOLINE_SMARTS,
    TASK19_SKIPPED_REACTIONS,
    TASK19_TOTAL_REACTIONS,
    TASK19_VALID_REACTIONS,
)
...
REACTION_KEY = "quinoline_construction"
TASK_LABEL = (
    "Reactions that construct a quinoline ring system from non-quinoline-containing reactants"
)
TASK_DESCRIPTION = (
    "A reaction matches when at least one product molecule contains a quinoline "
    "ring substructure and no reactant molecule contains quinoline. Ignore reagents."
)
...
def maybe_init_tracing() -> None:
    ...
    initialized = init_tracing(
        project_name="CodeAct-Task19_tier3",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CodeAct task 19 evaluation.")
...
def build_question() -> str:
    ...
    - Ignore reagents.
    - A reaction matches when at least one product molecule contains a quinoline substructure and no reactant molecule contains quinoline.
    - Reactions where quinoline is already present among reactants do not match, even if the product also contains quinoline.
...
async def main(model_name: str, context_size: int) -> None:
    maybe_init_tracing()
    tracer = get_tracer("codeact-task19")
    ...
    run_session_id = f"codeact-task19-{uuid.uuid4()}"
    question = build_question()
    full_gt_set = set(TASK19_HARDCODED_GROUND_TRUTH_INDICES)

    print(
        "Ground truth [quinoline_construction] "
        f"count={TASK19_POSITIVE_REACTIONS} "
        f"valid={TASK19_VALID_REACTIONS} "
        f"skipped={TASK19_SKIPPED_REACTIONS} "
        f"definition={TASK19_GROUND_TRUTH_DEFINITION} "
        f"smarts={TASK19_QUINOLINE_SMARTS}"
    )

    run = wandb.init(
        project="CodeAct-Task19_tier3",
        config={
            ...
            "ground_truth_count": TASK19_POSITIVE_REACTIONS,
            "ground_truth_total_reactions": TASK19_TOTAL_REACTIONS,
            "ground_truth_valid_reactions": TASK19_VALID_REACTIONS,
            "ground_truth_skipped_reactions": TASK19_SKIPPED_REACTIONS,
            "ground_truth_quinoline_smarts": TASK19_QUINOLINE_SMARTS,
            "ground_truth_definition": TASK19_GROUND_TRUTH_DEFINITION,
        },
    )
    ...
    """
    print("Question 1/1 task=quinoline_construction")
    ...
    with tracer.start_as_current_span("codeact_task19_sample_0") as sample_span:
        ...
        with using_tracing_attributes(
            ...
            tags=["codeact", "sample", "task19_quinoline_construction"],
        ):
    ...
    print(f"Predicted [quinoline_construction] count: {predicted_count}")
    print(f"Ground truth [quinoline_construction] count: {ground_truth_count}")
    print(
        "Metrics [quinoline_construction] -> "
    ...
    run.summary["ground_truth/quinoline_construction/count"] = ground_truth_count
    run.summary["ground_truth/quinoline_construction/full_count"] = len(full_gt_set)
    run.summary["ground_truth/quinoline_smarts"] = TASK19_QUINOLINE_SMARTS
    run.summary["ground_truth/definition"] = TASK19_GROUND_TRUTH_DEFINITION
    run.summary["ground_truth/total_reactions"] = TASK19_TOTAL_REACTIONS
    run.summary["ground_truth/valid_reactions"] = TASK19_VALID_REACTIONS
    run.summary["ground_truth/skipped_reactions"] = TASK19_SKIPPED_REACTIONS
