...
from task16_hardcoded_ground_truth import (
    TASK16_HARDCODED_GROUND_TRUTH_INDICES,
    TASK16_POSITIVE_REACTIONS,
    TASK16_PYRIDINE_SMARTS,
    TASK16_SKIPPED_REACTIONS,
    TASK16_TOTAL_REACTIONS,
    TASK16_VALID_REACTIONS,
)
...
TASK_LABEL = "Reactions that produce a product containing a pyridine ring"
TASK_DESCRIPTION = (
    "A reaction matches when at least one product molecule contains a pyridine "
    "ring substructure. Ignore reactants and reagents."
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run LLM task 16 pyridine ring product recognition index evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
def build_question() -> str:
    ...
    Guidance:
    - Ignore reactants and reagents; examine product molecules only.
    - A reaction matches when at least one product molecule contains a pyridine ring.
...
async def main(model_name: str, context_size: int) -> None:
    ...
    full_gt_set = set(TASK16_HARDCODED_GROUND_TRUTH_INDICES)

    print(
        "Ground truth [pyridine_product] "
        f"count={TASK16_POSITIVE_REACTIONS} "
        f"valid={TASK16_VALID_REACTIONS} "
        f"skipped={TASK16_SKIPPED_REACTIONS} "
        f"smarts={TASK16_PYRIDINE_SMARTS}"
    )
    ...
    run = wandb.init(
        ...
        config={
            ...
            "ground_truth_count": TASK16_POSITIVE_REACTIONS,
            "ground_truth_total_reactions": TASK16_TOTAL_REACTIONS,
            "ground_truth_valid_reactions": TASK16_VALID_REACTIONS,
            "ground_truth_skipped_reactions": TASK16_SKIPPED_REACTIONS,
            "ground_truth_pyridine_smarts": TASK16_PYRIDINE_SMARTS,
            "ground_truth_definition": (
                f"at least one product matches pyridine SMARTS {TASK16_PYRIDINE_SMARTS}"
            ),
    ...
    """
    print("Question 1/1 task=pyridine_product")
    ...
    with using_tracing_attributes(
        ...
        metadata={
            "sample_index": 0,
            "sample_count": 1,
            "task": "pyridine_product",
            "agent": "llm_baseline",
        },
        tags=["llm-baseline", "sample", "task16_pyridine_product"],
    ):
    ...
    print(f"Predicted [pyridine_product] count: {predicted_count}")
    print(f"Ground truth [pyridine_product] count: {ground_truth_count}")
    print(
        "Metrics [pyridine_product] -> "
    ...
    wandb.log(
        {
            "sample_idx": 0,
            "sample/0/reaction_key": "pyridine_product",
    ...
    run.summary["ground_truth/pyridine_product/count"] = ground_truth_count
    run.summary["ground_truth/pyridine_product/full_count"] = len(full_gt_set)
    run.summary["ground_truth/pyridine_smarts"] = TASK16_PYRIDINE_SMARTS
    run.summary["ground_truth/total_reactions"] = TASK16_TOTAL_REACTIONS
    run.summary["ground_truth/valid_reactions"] = TASK16_VALID_REACTIONS
    run.summary["ground_truth/skipped_reactions"] = TASK16_SKIPPED_REACTIONS
