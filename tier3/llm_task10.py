...
def build_question(reaction_key: str) -> str:
    return build_task10_question(reaction_key, allow_code=False)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run LLM task 10 mechanism-family index evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
async def main(model_name: str, context_size: int) -> None:
    ...
    run_session_id = f"llm-task10-{uuid.uuid4()}"
    ...
    async def _evaluate_question(_item):
        ...
        with using_tracing_attributes(
            ...
            tags=["llm-baseline", "sample", "task10_mechanism_families"],
        ):
