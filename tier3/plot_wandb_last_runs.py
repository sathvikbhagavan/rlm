    ...
    *,
    ...
    if nrows == 1:
        fig, axes = plt.subplots(
            nrows, ncols, figsize=(3.4 * ncols, 4.6), sharey=True
        )
    else:
    ...
    for index, (ax, (name, task_ids)) in enumerate(
        zip(axes_flat, subcategories)
    ):
    ...
    if nrows == 1:
        fig.subplots_adjust(top=0.90, bottom=0.20, left=0.10, right=0.99, wspace=0.22)
        fig.suptitle(
            f"{tier_label} sub-category F1 vs context ({model_name})",
            y=0.97,
            fontsize=13,
        )
        shared_family_legend(fig, y=0.06)
    else:
        fig.subplots_adjust(
...
def plot_overall_f1_line(
    ...
    plot_f1_lines_on_ax(
        ax,
        overall,
        title=f"Tier3 overall F1 vs context ({model_name})",
        show_legend=True,
    )
