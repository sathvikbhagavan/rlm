...
TASK_LABEL = "Net C-N bond formation by connectivity delta"
TASK_DESCRIPTION = (
    "A reaction forms a C-N bond when the total number of carbon-nitrogen "
    "connections in the products is greater than the total number of "
    "carbon-nitrogen connections in the reactants. Count connectivity only: "
    "single, double, triple, and aromatic C-N bonds each count as one C-N "
...
class GroundTruthResult:
    ...
    results_by_index: dict[int, CNConnectivityResult]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run RLM task 12 C-N bond formation connectivity evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
def print_ground_truth_diagnostics(
    ...
    print(f"Ground truth C-N formation count: {gt_result.positive_reactions}")
    ...
    for idx in positives:
        ...
        print(
            f"{idx}: reactant_cn={result.reactant_cn_count} "
            f"product_cn={result.product_cn_count} delta={result.delta}"
        )
...
def main(model_name: str, dataset_path: str, ground_truth_only: bool, spot_check_limit: int) -> None:
    ...
    run = wandb.init(
        ...
        config={
            ...
            "ground_truth_definition": "product C-N connectivity count - reactant C-N connectivity count > 0",
        },
    )
    ...
    with using_tracing_attributes(
        ...
        tags=["run_rlms", "sample", "task12_cn_bond_formation"],
    ):
    ...
    print(f"Response [cn_bond_formation]: {response}")
    ...
    # run.summary["ground_truth/cn_bond_formation/indices"] = ",".join(str(x) for x in gt_indices)
