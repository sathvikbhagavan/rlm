...
REACTION_KEY = "co_bond_breaking_connectivity_delta"
REACTION_LABEL = "Net C-O bond breaking by connectivity delta"
REACTION_DESCRIPTION = (
    "A reaction breaks a C-O bond when the total number of carbon-oxygen "
    "connections in products is less than in reactants. Count connectivity only: "
    "single, double, triple, and aromatic C-O bonds each count as one C-O "
    "connection. Ignore reagents."
)
...
def build_question() -> str:
    ...
    - Count all C-O bond types (single/double/triple/aromatic) as one connection each.
    - Ignore reagents (middle field).
    - A reaction matches when product-side C-O connections minus reactant-side C-O connections is < 0.
...
async def main(
    ...
    """
    print("Question 1/1 task=co_bond_breaking_connectivity_delta")
    print(
    ...
    with tracer.start_as_current_span("codeact_task13_sample_0") as sample_span:
        ...
        with using_tracing_attributes(
            ...
            tags=["codeact", "sample", "task13_co_connectivity"],
        ):
    ...
    run.summary["full_ground_truth/co_bond_breaking/count"] = len(full_gt_indices)
    run.summary["full_ground_truth/co_bond_breaking/indices"] = ",".join(str(x) for x in full_gt_indices)
    wandb.finish()
