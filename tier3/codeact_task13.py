...
from task13_hardcoded_ground_truth import (
    TASK13_HARDCODED_GROUND_TRUTH_INDICES,
    TASK13_POSITIVE_REACTIONS,
    TASK13_SKIPPED_REACTIONS,
    TASK13_TOTAL_REACTIONS,
    TASK13_VALID_REACTIONS,
)
...
REACTION_KEY = "new_nitrogen_atom"
TASK_LABEL = "Reactions that introduce at least one new nitrogen atom"
TASK_DESCRIPTION = (
    "A reaction introduces at least one new nitrogen atom when the total number of "
    "nitrogen atoms in the products is greater than the total number of nitrogen "
    "atoms in the reactants. Ignore the reagent field."
)
...
def maybe_init_tracing() -> None:
    ...
    initialized = init_tracing(
        project_name="CodeAct-Task13_tier3",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CodeAct task 13 evaluation.")
...
def build_question() -> str:
    ...
    Guidance:
    - Use RDKit for all parsing and atom counting.
    - Convert reactants and products to RDKit molecules; do not count by string matching.
    - Ignore reagents in the middle field.
    - A nitrogen atom is any atom whose element is nitrogen.
    - A reaction matches when product-side nitrogen atom count minus reactant-side nitrogen atom count is greater than zero.
...
async def main(model_name: str, context_size: int) -> None:
    ...
    full_gt_set = set(TASK13_HARDCODED_GROUND_TRUTH_INDICES)

    print(
        "Ground truth [new_nitrogen_atom] "
        f"count={TASK13_POSITIVE_REACTIONS} "
        f"valid={TASK13_VALID_REACTIONS} "
        f"skipped={TASK13_SKIPPED_REACTIONS}"
    )

    run = wandb.init(
        project="CodeAct-Task13_tier3",
        config={
            ...
            "ground_truth_count": TASK13_POSITIVE_REACTIONS,
            "ground_truth_total_reactions": TASK13_TOTAL_REACTIONS,
            "ground_truth_valid_reactions": TASK13_VALID_REACTIONS,
            "ground_truth_skipped_reactions": TASK13_SKIPPED_REACTIONS,
            "ground_truth_definition": "count(N in products) > count(N in reactants)",
        },
    )
    ...
    """
    print("Question 1/1 task=new_nitrogen_atom")
    ...
    with tracer.start_as_current_span("codeact_task13_sample_0") as sample_span:
        ...
        with using_tracing_attributes(
            ...
            tags=["codeact", "sample", "task13_new_nitrogen_atom"],
        ):
    ...
    print(f"Predicted [new_nitrogen_atom] count: {predicted_count}")
    print(f"Ground truth [new_nitrogen_atom] count: {ground_truth_count}")
    print(
        "Metrics [new_nitrogen_atom] -> "
    ...
    run.summary["ground_truth/new_nitrogen_atom/count"] = ground_truth_count
    run.summary["ground_truth/new_nitrogen_atom/full_count"] = len(full_gt_set)
    run.summary["ground_truth/total_reactions"] = TASK13_TOTAL_REACTIONS
    run.summary["ground_truth/valid_reactions"] = TASK13_VALID_REACTIONS
    run.summary["ground_truth/skipped_reactions"] = TASK13_SKIPPED_REACTIONS
