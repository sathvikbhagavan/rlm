...
def maybe_init_tracing() -> None:
    ...
    initialized = init_tracing(
        project_name="CodeAct-Task22_tier3",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CodeAct task 22 evaluation.")
...
async def main(model_name: str, context_size: int) -> None:
    maybe_init_tracing()
    tracer = get_tracer("codeact-task22")
    ...
    run_session_id = f"codeact-task22-{uuid.uuid4()}"
    question = build_question()
    full_gt_set = set(TASK22_HARDCODED_GROUND_TRUTH_INDICES)
    ...
    with tracer.start_as_current_span("codeact_task22_sample_0") as sample_span:
        ...
        with using_tracing_attributes(
            ...
            tags=["codeact", "sample", "task22_hatu_pf6_or_t3p_reagent"],
        ):
    ...
    run.summary["ground_truth/hatu_pf6_or_t3p_reagent/full_count"] = len(full_gt_set)
