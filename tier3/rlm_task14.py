...
TASK_LABEL = "Net nitrogen introduction by atom-count delta"
TASK_DESCRIPTION = (
    "A reaction introduces new nitrogen into the product when the total number "
    "of nitrogen atoms in the products is greater than the total number of "
    "nitrogen atoms in the reactants. Count nitrogen atoms by RDKit atomic "
    "number, and ignore the reagent field."
)
...
class GroundTruthResult:
    ...
    results_by_index: dict[int, NitrogenIntroductionResult]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run RLM task 14 nitrogen introduction atom-count evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
def build_question() -> str:
    ...
    - For each molecule, iterate through atoms with RDKit.
...
def print_ground_truth_diagnostics(
    ...
    print(f"Ground truth nitrogen introduction count: {gt_result.positive_reactions}")
    ...
    for idx in positives:
        ...
        print(
            f"{idx}: reactant_n={result.reactant_n_count} "
            f"product_n={result.product_n_count} delta={result.delta}"
        )
...
def main(model_name: str, dataset_path: str, ground_truth_only: bool, spot_check_limit: int) -> None:
    ...
    print(
        "Ground truth [nitrogen_introduction] "
    ...
    run = wandb.init(
        ...
        config={
            ...
            "ground_truth_definition": "product nitrogen atom count - reactant nitrogen atom count > 0",
        },
    )
    ...
    print("Question 1/1 task=nitrogen_introduction")
    ...
    with using_tracing_attributes(
        ...
        metadata={
            "sample_index": 0,
            "sample_count": 1,
            "task": "nitrogen_introduction",
            "ground_truth_definition": "atom_count_delta",
        },
        tags=["run_rlms", "sample", "task14_nitrogen_introduction"],
    ):
    ...
    print(f"Response [nitrogen_introduction]: {response}")
    print(f"Predicted [nitrogen_introduction] count: {predicted_count}")
    print(f"Ground truth [nitrogen_introduction] count: {ground_truth_count}")
    print(
        "Metrics [nitrogen_introduction] -> "
    ...
    if iteration_metrics:
        ...
        wandb.log(
            {
                "sample_idx": 0,
                "sample/0/reaction_key": "nitrogen_introduction",
    ...
    run.summary["ground_truth/nitrogen_introduction/count"] = ground_truth_count
    # run.summary["ground_truth/nitrogen_introduction/indices"] = ",".join(
