...
REACTION_KEY = "new_ring_construction"
TASK_LABEL = (
...
def maybe_init_tracing() -> None:
    ...
    initialized = init_tracing(
        project_name="CodeAct-Task18_tier3",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CodeAct task 18 evaluation.")
...
def build_question() -> str:
    ...
    Guidance:
    - Use RDKit for all parsing and ring-system comparison.
    - Parse reactants and products into separate RDKit molecules; do not count by string matching.
    - Sanitize all molecules before comparison so aromaticity is perceived consistently.
    - Ignore reagents.
...
async def main(model_name: str, context_size: int) -> None:
    maybe_init_tracing()
    tracer = get_tracer("codeact-task18")
    ...
    run_session_id = f"codeact-task18-{uuid.uuid4()}"
    ...
    with tracer.start_as_current_span("codeact_task18_sample_0") as sample_span:
        ...
        with using_tracing_attributes(
            ...
            tags=["codeact", "sample", "task18_new_ring_construction"],
        ):
