...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run LLM task 15 exactly-one C-C bond formation index evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
def build_question() -> str:
    ...
    - Bond type matters: single, double, triple, and aromatic C-C bonds are distinct types.
...
async def main(model_name: str, context_size: int) -> None:
    ...
    with using_tracing_attributes(
        ...
        tags=["llm-baseline", "sample", "task15_exactly_one_cc_formed"],
    ):
