...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run LLM task 19 quinoline construction index evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
def build_question() -> str:
    ...
    - A reaction matches when at least one product molecule contains a quinoline ring and no reactant molecule contains quinoline.
    - Reactions where quinoline is already present among reactants do not match.
...
async def main(model_name: str, context_size: int) -> None:
    ...
    run_session_id = f"llm-task19-{uuid.uuid4()}"
    ...
    with using_tracing_attributes(
        ...
        tags=["llm-baseline", "sample", "task19_quinoline_construction"],
    ):
