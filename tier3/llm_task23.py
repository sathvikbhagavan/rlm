...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run LLM task 23 stereocenter-from-achiral-reactants index evaluation."
    )
...
async def main(model_name: str, context_size: int) -> None:
    ...
    run_session_id = f"llm-task23-{uuid.uuid4()}"
    ...
    with using_tracing_attributes(
        ...
        tags=["llm-baseline", "sample", "task23_stereocenter_from_achiral_reactants"],
    ):
