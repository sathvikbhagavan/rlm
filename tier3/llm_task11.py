...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run LLM task 11 C-N bond formation index evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
def build_question() -> str:
    ...
    - A C-N connection is any bond where one endpoint atom is carbon and the other is nitrogen.
...
async def main(model_name: str, context_size: int) -> None:
    ...
    with using_tracing_attributes(
        ...
        tags=["llm-baseline", "sample", "task11_cn_bond_formation"],
    ):
