...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run CodeAct task 10b mechanism-family evaluation."
    )
...
async def main(model_name: str, context_size: int) -> None:
    maybe_init_tracing()
    tracer = get_tracer("codeact-task10b")
    ...
    run_session_id = f"codeact-task10b-{uuid.uuid4()}"
    ...
    async def _evaluate_question(_item):
        ...
        with tracer.start_as_current_span(f"codeact_task10b_sample_{i}") as sample_span:
            ...
            with using_tracing_attributes(
                ...
                tags=["codeact", "sample", "task10b_mechanism_families"],
            ):
