...
REACTION_KEY = "fused_heterocycle_product"
...
def maybe_init_tracing() -> None:
    ...
    initialized = init_tracing(
        project_name="CodeAct-Task17_tier3",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CodeAct task 17 evaluation.")
...
async def main(model_name: str, context_size: int) -> None:
    maybe_init_tracing()
    tracer = get_tracer("codeact-task17")
    ...
    run_session_id = f"codeact-task17-{uuid.uuid4()}"
    question = build_question()
    full_gt_set = set(TASK17_HARDCODED_GROUND_TRUTH_INDICES)
    ...
    with tracer.start_as_current_span("codeact_task17_sample_0") as sample_span:
        ...
        with using_tracing_attributes(
            ...
            tags=["codeact", "sample", "task17_fused_heterocycle_product"],
        ):
    ...
    run.summary["ground_truth/fused_heterocycle_product/full_count"] = len(full_gt_set)
