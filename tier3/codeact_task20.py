...
from task20_hardcoded_ground_truth import (
    TASK20_GROUND_TRUTH_DEFINITION,
    TASK20_HARDCODED_GROUND_TRUTH_INDICES,
    TASK20_POSITIVE_REACTIONS,
    TASK20_SKIPPED_REACTIONS,
    TASK20_TOTAL_REACTIONS,
    TASK20_VALID_REACTIONS,
)
...
REACTION_KEY = "fused_ring_construction"
TASK_LABEL = (
    "Reactions that construct a fused bicyclic or polycyclic ring system "
    "from non-fused reactants"
)
TASK_DESCRIPTION = (
    "A reaction matches when at least one product molecule contains two or more "
    "rings that share at least one bond (a fused bicyclic or polycyclic scaffold), "
    "and no reactant molecule contains such a fused ring system. Ignore reagents."
)
TASK_EVALUATION_GUIDANCE = """
    Fused ring systems:
    - A molecule has a fused bicyclic or polycyclic ring system when at least one bond is shared by two or more smallest-set-of-smallest-rings (SSSR).
    - Equivalently, two or more SSSR rings share at least two atoms (the atoms of a fusion bond).

    When a product counts as fused:
    - Bicyclic or polycyclic assemblies where rings are joined along a shared bond (e.g. naphthalene, decalin, quinoline, bridged bicyclics).

    When a molecule does NOT count as fused:
    - Monocyclic molecules.
    - Spiro systems (rings share only one atom, not a bond).
    - Linked ring systems with no shared ring bond (e.g. biphenyl: two rings connected by an inter-ring single bond only).

    Matching rule:
    - A reaction matches when at least one product molecule has a fused ring system and no reactant molecule has a fused ring system.
    - Reagents are ignored.
"""
...
def maybe_init_tracing() -> None:
    ...
    initialized = init_tracing(
        project_name="CodeAct-Task20_tier3",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CodeAct task 20 evaluation.")
...
def build_question() -> str:
    ...
    Guidance:
    - Use RDKit for all parsing and ring analysis.
    - Parse each reaction's reactant and product sides into separate RDKit molecules; do not count by string matching.
    - Ignore reagents.
    - Apply the evaluation assumptions below when deciding whether a molecule has a fused ring system.
{TASK_EVALUATION_GUIDANCE}
...
async def main(model_name: str, context_size: int) -> None:
    maybe_init_tracing()
    tracer = get_tracer("codeact-task20")
    ...
    run_session_id = f"codeact-task20-{uuid.uuid4()}"
    question = build_question()
    full_gt_set = set(TASK20_HARDCODED_GROUND_TRUTH_INDICES)

    print(
        "Ground truth [fused_ring_construction] "
        f"count={TASK20_POSITIVE_REACTIONS} "
        f"valid={TASK20_VALID_REACTIONS} "
        f"skipped={TASK20_SKIPPED_REACTIONS} "
        f"definition={TASK20_GROUND_TRUTH_DEFINITION}"
    )

    run = wandb.init(
        project="CodeAct-Task20_tier3",
        config={
            ...
            "ground_truth_count": TASK20_POSITIVE_REACTIONS,
            "ground_truth_total_reactions": TASK20_TOTAL_REACTIONS,
            "ground_truth_valid_reactions": TASK20_VALID_REACTIONS,
            "ground_truth_skipped_reactions": TASK20_SKIPPED_REACTIONS,
            "ground_truth_definition": TASK20_GROUND_TRUTH_DEFINITION,
        },
    )
    ...
    """
    print("Question 1/1 task=fused_ring_construction")
    ...
    with tracer.start_as_current_span("codeact_task20_sample_0") as sample_span:
        ...
        with using_tracing_attributes(
            ...
            tags=["codeact", "sample", "task20_fused_ring_construction"],
        ):
    ...
    print(f"Predicted [fused_ring_construction] count: {predicted_count}")
    print(f"Ground truth [fused_ring_construction] count: {ground_truth_count}")
    print(
        "Metrics [fused_ring_construction] -> "
    ...
    run.summary["ground_truth/fused_ring_construction/count"] = ground_truth_count
    run.summary["ground_truth/fused_ring_construction/full_count"] = len(full_gt_set)
    run.summary["ground_truth/definition"] = TASK20_GROUND_TRUTH_DEFINITION
    run.summary["ground_truth/total_reactions"] = TASK20_TOTAL_REACTIONS
    run.summary["ground_truth/valid_reactions"] = TASK20_VALID_REACTIONS
    run.summary["ground_truth/skipped_reactions"] = TASK20_SKIPPED_REACTIONS
