...
def maybe_init_tracing() -> None:
    ...
    initialized = init_tracing(
        project_name="RLMs-Task20_tier3",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run RLM task 20 fused ring construction index evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
def main(model_name: str, context_size: int) -> None:
    ...
    gt_indices = TASK20_HARDCODED_GROUND_TRUTH_INDICES
    ...
    with using_tracing_attributes(
        ...
        metadata={
            "sample_index": 0,
            "sample_count": 1,
            "task": "fused_ring_construction",
            "ground_truth_definition": TASK20_GROUND_TRUTH_DEFINITION,
        },
        tags=["run_rlms", "sample", "task20_fused_ring_construction"],
    ):
    ...
    if iteration_metrics:
        ...
        wandb.log(
            {
                "sample_idx": 0,
                "sample/0/reaction_key": "fused_ring_construction",
    ...
    run.summary["ground_truth/fused_ring_construction/full_count"] = len(gt_set)
