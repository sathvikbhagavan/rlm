...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run LLM task 20 fused ring construction index evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
def build_question() -> str:
    ...
    - A fused ring system means two or more rings share at least one bond (bicyclic or polycyclic fusion).
    - Spiro systems (rings sharing only one atom) do not count as fused.
    - Linked ring systems without a shared ring bond (e.g. biphenyl) do not count as fused.
    - A reaction matches when at least one product has a fused ring system and no reactant does.
...
async def main(model_name: str, context_size: int) -> None:
    ...
    run_session_id = f"llm-task20-{uuid.uuid4()}"
    ...
    with using_tracing_attributes(
        ...
        tags=["llm-baseline", "sample", "task20_fused_ring_construction"],
    ):
