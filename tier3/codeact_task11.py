...
REACTION_KEY = "cc_bond_formation_connectivity_delta"
REACTION_LABEL = "Net C-C bond formation by connectivity delta"
REACTION_DESCRIPTION = (
    "A reaction forms a C-C bond when the total number of carbon-carbon connections "
    "in products is greater than in reactants. Count connectivity only: single, "
    "double, triple, and aromatic C-C bonds each count as one C-C connection. "
    "Ignore reagents."
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CodeAct task 11 evaluation.")
...
def build_question() -> str:
    ...
    - Count C-C connectivity, not bond order magnitude.
    - A C-C connection is any bond where both endpoint atoms are carbon.
    - Count all C-C bond types (single/double/triple/aromatic) as one connection each.
    - Ignore reagents (middle field).
    - A reaction matches when product-side C-C connections minus reactant-side C-C connections is > 0.
...
async def main(
    ...
    """
    print("Question 1/1 task=cc_bond_formation_connectivity_delta")
    print(
    ...
    with tracer.start_as_current_span("codeact_task11_sample_0") as sample_span:
        ...
        with using_tracing_attributes(
            ...
            tags=["codeact", "sample", "task11_cc_connectivity"],
        ):
    ...
    run.summary["full_ground_truth/cc_bond_formation/count"] = len(full_gt_indices)
    run.summary["full_ground_truth/cc_bond_formation/indices"] = ",".join(str(x) for x in full_gt_indices)
    wandb.finish()
