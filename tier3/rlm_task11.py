...
TASK_LABEL = "Net C-C bond formation by connectivity delta"
TASK_DESCRIPTION = (
    "A reaction forms a C-C bond when the total number of carbon-carbon "
    "connections in the products is greater than the total number of "
    "carbon-carbon connections in the reactants. Count connectivity only: "
    "single, double, triple, and aromatic C-C bonds each count as one C-C "
    "connection. Ignore the reagent field."
)


@dataclass
class CCConnectivityResult:
    index: int
    reactant_cc_count: int
    product_cc_count: int
    delta: int
    is_valid: bool
    error: str | None = None
...
class GroundTruthResult:
    ...
    results_by_index: dict[int, CCConnectivityResult]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run RLM task 11 C-C bond formation connectivity evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
def count_cc_connections(mols: list[Chem.Mol]) -> int:
    count = 0
    for mol in mols:
...
def analyze_cc_connectivity(indexed_line: str) -> CCConnectivityResult:
    idx = -1
    try:
        ...
        reactant_cc_count = count_cc_connections(reactant_mols)
        product_cc_count = count_cc_connections(product_mols)
        delta = product_cc_count - reactant_cc_count
        return CCConnectivityResult(
            index=idx,
            reactant_cc_count=reactant_cc_count,
            product_cc_count=product_cc_count,
            delta=delta,
            is_valid=True,
        )
...
def ground_truth_indices(lines: list[str]) -> GroundTruthResult:
    ...
    for line in lines:
        result = analyze_cc_connectivity(line)
        if result.index >= 0:
...
def build_question() -> str:
    ...
    - A C-C connection is any bond where both endpoint atoms are carbon atoms.
    - Count connectivity, not bond order: single, double, triple, and aromatic C-C bonds each count as one.
    - A reaction matches when product-side C-C connections minus reactant-side C-C connections is greater than zero.
...
def print_ground_truth_diagnostics(
    ...
    print(f"Ground truth C-C formation count: {gt_result.positive_reactions}")
    ...
    for idx in positives:
        ...
        print(
            f"{idx}: reactant_cc={result.reactant_cc_count} "
            f"product_cc={result.product_cc_count} delta={result.delta}"
        )
...
def main(model_name: str, dataset_path: str, ground_truth_only: bool, spot_check_limit: int) -> None:
    ...
    print(
        "Ground truth [cc_bond_formation] "
    ...
    run = wandb.init(
        ...
        config={
            ...
            "ground_truth_definition": "product C-C connectivity count - reactant C-C connectivity count > 0",
        },
    )
    ...
    print("Question 1/1 task=cc_bond_formation")
    ...
    with using_tracing_attributes(
        ...
        metadata={
            "sample_index": 0,
            "sample_count": 1,
            "task": "cc_bond_formation",
            "ground_truth_definition": "connectivity_delta",
        },
        tags=["run_rlms", "sample", "task11_cc_bond_formation"],
    ):
    ...
    print(f"Response [cc_bond_formation]: {response}")
    print(f"Predicted [cc_bond_formation] count: {predicted_count}")
    print(f"Ground truth [cc_bond_formation] count: {ground_truth_count}")
    print(
        "Metrics [cc_bond_formation] -> "
    ...
    if iteration_metrics:
        ...
        wandb.log(
            {
                "sample_idx": 0,
                "sample/0/reaction_key": "cc_bond_formation",
    ...
    run.summary["ground_truth/cc_bond_formation/count"] = ground_truth_count
    run.summary["ground_truth/cc_bond_formation/indices"] = ",".join(str(x) for x in gt_indices)
