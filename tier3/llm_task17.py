...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run LLM task 17 multi-class fused heterocycle recognition evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
async def main(model_name: str, context_size: int) -> None:
    ...
    run_session_id = f"llm-task17-{uuid.uuid4()}"
    ...
    with using_tracing_attributes(
        ...
        tags=["llm-baseline", "sample", "task17_fused_heterocycle_product"],
    ):
