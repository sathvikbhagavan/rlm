...
from task21_hardcoded_ground_truth import (
    TASK21_GROUND_TRUTH_DEFINITION,
    TASK21_HARDCODED_GROUND_TRUTH_INDICES,
    TASK21_POSITIVE_REACTIONS,
    TASK21_SKIPPED_REACTIONS,
    TASK21_TOTAL_REACTIONS,
    TASK21_VALID_REACTIONS,
)
...
TASK_LABEL = "Reactions whose reagent slot contains a transition metal"
TASK_DESCRIPTION = (
    "A reaction matches when the middle reagent slot contains at least one molecule "
    "with at least one atom from the explicitly defined transition-metal set. "
    "Transition metals appearing only in reactants or products do not count."
)
TASK_EVALUATION_GUIDANCE = f"""
    Transition-metal definition:
    - For this task, transition metals are the elements in these ranges: Sc-Zn, Y-Cd, Hf-Hg, and Rf-Cn.
    - Do not include La, Ac, lanthanides, actinides, alkali metals, alkaline-earth metals, or main-group metals.

    Matching rule:
    - Inspect only the reagent slot, i.e. the middle field in reactants>reagents>products.
    - Count a reaction if any reagent-slot molecule contains at least one atom from the defined transition-metal set.
    - Do not count reactions where such a metal appears only on the reactant or product side.
...
def maybe_init_tracing() -> None:
    ...
    initialized = init_tracing(
        project_name="CodeAct-Task21_tier3",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CodeAct task 21 evaluation.")
...
def build_question() -> str:
    ...
    Guidance:
    - Use RDKit for parsing molecules and checking atom atomic numbers.
    - Do not use string matching alone; metal atoms can appear in charged or coordinated forms.
...
async def main(model_name: str, context_size: int) -> None:
    maybe_init_tracing()
    tracer = get_tracer("codeact-task21")
    ...
    run_session_id = f"codeact-task21-{uuid.uuid4()}"
    question = build_question()
    full_gt_set = set(TASK21_HARDCODED_GROUND_TRUTH_INDICES)

    print(
        "Ground truth [transition_metal_reagent] "
        f"count={TASK21_POSITIVE_REACTIONS} "
        f"valid={TASK21_VALID_REACTIONS} "
        f"skipped={TASK21_SKIPPED_REACTIONS} "
        f"definition={TASK21_GROUND_TRUTH_DEFINITION}"
    )

    run = wandb.init(
        project="CodeAct-Task21_tier3",
        config={
            ...
            "ground_truth_count": TASK21_POSITIVE_REACTIONS,
            "ground_truth_total_reactions": TASK21_TOTAL_REACTIONS,
            "ground_truth_valid_reactions": TASK21_VALID_REACTIONS,
            "ground_truth_skipped_reactions": TASK21_SKIPPED_REACTIONS,
            "ground_truth_definition": TASK21_GROUND_TRUTH_DEFINITION,
        },
    )
    ...
    """
    print("Question 1/1 task=transition_metal_reagent")
    ...
    with tracer.start_as_current_span("codeact_task21_sample_0") as sample_span:
        ...
        with using_tracing_attributes(
            ...
            tags=["codeact", "sample", "task21_transition_metal_reagent"],
        ):
    ...
    print(f"Predicted [transition_metal_reagent] count: {predicted_count}")
    print(f"Ground truth [transition_metal_reagent] count: {ground_truth_count}")
    print(
        "Metrics [transition_metal_reagent] -> "
    ...
    run.summary["ground_truth/transition_metal_reagent/count"] = ground_truth_count
    run.summary["ground_truth/transition_metal_reagent/full_count"] = len(full_gt_set)
    run.summary["ground_truth/definition"] = TASK21_GROUND_TRUTH_DEFINITION
    run.summary["ground_truth/total_reactions"] = TASK21_TOTAL_REACTIONS
    run.summary["ground_truth/valid_reactions"] = TASK21_VALID_REACTIONS
    run.summary["ground_truth/skipped_reactions"] = TASK21_SKIPPED_REACTIONS
