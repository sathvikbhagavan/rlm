...
async def main(model_name: str, context_size: int) -> None:
    maybe_init_tracing()
    tracer = get_tracer("codeact-task2")
    ...
    ground_truth_indices_by_threshold: dict[int, set[int]] = {
        threshold: set(TASK2_HARDCODED_GROUND_TRUTH_INDICES.get(threshold, []))
    ...
    for i, threshold in enumerate(THRESHOLDS):
        ...
        with tracer.start_as_current_span(f"codeact_task2_sample_{i}") as sample_span:
