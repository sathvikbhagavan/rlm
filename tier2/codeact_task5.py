...
ENABLE_TRACING = True
WORKFLOW_TIMEOUT_S = codeact_workflow_timeout(default=600.0)
...
async def main(model_name: str, context_size: int) -> None:
    maybe_init_tracing()
    tracer = get_tracer("codeact-task5")
    ...
    async def _evaluate_question(_item):
        ...
        (i, (weight_threshold, ring_x)) = _item
        ...
        with tracer.start_as_current_span(f"codeact_task5_sample_{i}") as sample_span:
            sample_span.set_attributes(
                {
                    "sample.index": i,
                    "sample.count": len(combinations),
                    "sample.weight_threshold_da": weight_threshold,
                    "sample.ring_x": ring_x,
                    "agent.name": "codeact",
                }
            )
