...
def build(root: Path) -> dict[str, Any]:
    ...
    (output / "README.md").write_text(
        ...
        "This directory is rebuilt by `build_failure_analysis.py` and "
        "`plot_failure_analysis.py`.\n\n"
        f"The frozen five-model record contains {len(records):,} jobs. Excluding the "
        f"{len(records) - len(valid):,} unresolved main-arm score recoveries leaves "
        f"{len(valid):,} trace-eligible jobs and "
        f"{sum(int(row['question_count']) for row in valid):,} expected question "
        "trajectories. Terminal execution failures remain separate from scientific "
        "wrong answers.\n\n"
        "`job_outcomes.csv` covers the trace-eligible job population. "
        "`trace_error_records.csv` contains the "
        f"{len(signatures):,} corrected-task question outputs whose retained predictions "
        "support exact reconstruction of precision and recall. Its labels are observable "
        "set-error signatures, not inferred chemical causes. Outputs without sufficient "
